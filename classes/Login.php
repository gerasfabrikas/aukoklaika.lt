...
class Login
{
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_email, user_fname, user_lname, user_address, user_city, user_region, user_phone FROM users WHERE user_id = :user_id
			...
			$_SESSION['user_region'] = $result_row->user_region;
            ...
            //$this->errors[] = $this->lang['Invalid cookie']; //removed
        }
        return false;
    }
    ...
    public function sendPasswordResetMail($user_name, $user_email, $user_password_reset_hash)
    {
        ...
        $mail->set('Sender', EMAIL_PASSWORDRESET_FROM);
